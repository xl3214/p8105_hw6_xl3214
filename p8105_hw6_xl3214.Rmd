...
cv_folds <- crossv_mc(q3_for_analysis, 100) # 100 Monte Carlo resamples

# Function to fit a model and return RMSE
get_rmse <- function(model_formula, train_data, test_data) {
  model <- lm(model_formula, data = train_data)
  pred <- predict(model, newdata = test_data)
  # Explicitly use data.frame method to access 'bwt' column
  rmse <- sqrt(mean((test_data[["bwt"]] - pred)^2))
  return(rmse)
}

# Cross-validate models
rmse_length_ga <- map_dbl(cv_folds, ~get_rmse(bwt ~ blength + gaweeks, .x$train, .x$test))
rmse_head_length_sex_interactions <- map_dbl(cv_folds, ~get_rmse(bwt ~ bhead * blength * babysex, .x$train, .x$test))
rmse_stepwise <- map_dbl(cv_folds, ~get_rmse(bwt ~ babysex + bhead + blength + delwt + fincome + gaweeks + mheight + mrace + parity + ppwt + smoken, .x$train, .x$test))

# Calculate mean RMSE for each model
mean_rmse_length_ga <- mean(rmse_length_ga)
mean_rmse_head_length_sex_interactions <- mean(rmse_head_length_sex_interactions)
mean_rmse_stepwise <- mean(rmse_stepwise)

# Output Comparison Table
comparison_RMSE_df <- tibble(
  Model = c("Length + GA", "Head + Length + Sex + Interactions", "Current Stepwise Model"),
  Mean_RMSE = c(mean_rmse_length_ga, mean_rmse_head_length_sex_interactions, mean_rmse_stepwise)
)

comparison_RMSE_df |> knitr::kable()
```
